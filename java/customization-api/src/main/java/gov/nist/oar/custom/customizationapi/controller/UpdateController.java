...
/**
 * This is a webservice/restapi controller which gives options to access, update and delete the
 * record.
 * There are four end points provided in this, each dealing with specific tasks.
 * In OAR project internal landing page for the edi record is accessed using backed metadata.
 * This metadata is a advanced POD record called NERDm. In this api we are allowing the record to be modified by authorized user.
 * This webservice connects to backend MongoDB which holds the record being edited.
 * When the record is accessed for the first time, it is fetched from backend metadata service.
 * If it gets modified the updated record is saved in this stagging database until finalzed
 * Once it is finalized it is pushed back to backend service to merge and send to review.
 * 
 * @author Deoyani Nandrekar-Heinis
 *
 */
