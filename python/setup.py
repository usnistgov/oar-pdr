...
def write_version_mod(version):
    ...
    for pkg in [
        f
        for f in os.listdir(nistoardir)
...
def find_oar_metadata(submoddir="oar-metadata"):
    ...
    if not os.path.exists(outpy):
        msg = (
            "{0} subdirectory does apparently does not contain oar-metadata "
            "submodule"
        )
        raise RuntimeError(msg.format(out))
    return out
...
def build_oar_metadata(pkgdir, buildlib, buildscrp):
    ...
    cmd = "python setup.py build --build-lib={0} --build-scripts={1}".format(
        os.path.abspath(buildlib), os.path.abspath(buildscrp)
    ).split()
    if sys.executable:
    ...
    if ex != 0:
        raise RuntimeError(
            "oar-metadata submodule build failed; exit="
            + str(ex)
            + ";\ndir="
            + pydir
            + ";\ncmd="
...
setup(
    ...
    scripts=[
        "../scripts/ppmdserver.py",
        "../scripts/ppmdserver-uwsgi.py",
        "../scripts/preserver-uwsgi.py",
        "../scripts/pubserver-uwsgi.py",
        "../scripts/ppmdserver3-uwsgi.py",
        "../scripts/notify.py",
        "../scripts/pdr.py",
        "../scripts/pdr",
        "../scripts/pdrhealthcheck.py",
    ],
    packages=[
        "nistoar.pdr",
        "nistoar.pdr.publish",
        ...
        "nistoar.pdr.preserv.archive",
        "nistoar.pdr.preserv.archive.logs",
        "nistoar.pdr.preserv.archive.scripts",
        "nistoar.pdr.preserv.archive.tests",
        "nistoar.pdr.preserv.archive.schema",
        "nistoar.pdr.preserv.archive.config",
        "nistoar.testing",
    ],
    package_data={
        "nistoar.pdr": [
            "data/*",
            "preserv/archive/config/*.json",
            "preserv/archive/schema/*.json",
        ]
    },
